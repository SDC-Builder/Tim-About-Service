...
(async () => {
  try {
    ...
    let start = Date.now();
    let result = await client.execute(`select * from about where id = 9999897`);
    let now = Date.now();
    let miliSecondsElapsed = (now - start);
    // console.log(result);
    console.log(`elapsed miliseconds is ${miliSecondsElapsed}`);
    start = Date.now();
    result = await client.execute(`select * from about where id = 9999999`);
    now = Date.now();
    miliSecondsElapsed = (now - start);
    // console.log(result);
    console.log(`elapsed miliseconds is ${miliSecondsElapsed}`);
    start = Date.now();
    result = await client.execute(`select * from about where id = 9999987`);
    now = Date.now();
    miliSecondsElapsed = (now - start);
    // console.log(result);
    console.log(`elapsed miliseconds is ${miliSecondsElapsed}`);
    ...
    start = Date.now();
    result = await client.execute(`UPDATE about SET recent_views='1' where id = 9999987`);
    now = Date.now();
    miliSecondsElapsed = (now - start);
    console.log(result);
    console.log(`elapsed miliseconds is ${miliSecondsElapsed}`);
...
// ######################### Results #########################
// Tims-MBP:database timanderson$ node cassandraModel.js
// elapsed miliseconds is 3
// elapsed miliseconds is 1
// elapsed miliseconds is 2
